...
public class AirtelUtils {
    ...
    public AirtelPaymentRequestDTO convertToAirtelPaymentRequestDTO(String channelRequest, String clientCorrelationId, String msisdn,
        ...
        // Extract customData into a map
        Map<String, String> customDataMap = new HashMap<>();
        JSONArray customDataArray = channelRequestJson.getJSONArray("customData");
        for (int i = 0; i < customDataArray.length(); i++) {
            JSONObject data = customDataArray.getJSONObject(i);
            customDataMap.put(data.getString("key"), data.getString("value"));
        }
