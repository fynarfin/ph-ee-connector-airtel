...
public class InitiatePaymentWorker {
    ...
    public void setup() {
        zeebeClient.newWorker().jobType("initiate-payment").handler((client, job) -> {
            ...
            logger.info("THE CALLBACK URL IS {} ", variables.get(X_CALLBACKURL));
            logger.info("Request body is {}", airtelPaymentRequestDTO.toString());
            AirtelPaymentResponseDTO airtelPaymentResponseDTO = airtelService.initiateTransaction(airtelPaymentRequestDTO, country,
                    currency);

            logger.info("Response body is {}", airtelPaymentResponseDTO.toString());
            ...
            logger.info("THE RESPONSE CODE IS : {}", variables.get(RESPONSE_CODE));
            ...
            logger.info("THE TRANSACTION STATUS IS : {}", variables.get(TRANSACTION_STATUS_FAILED));
