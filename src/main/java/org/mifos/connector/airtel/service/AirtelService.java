...
public class AirtelService {
    ...
    public AirtelPaymentResponseDTO initiateTransaction(AirtelPaymentRequestDTO airtelPaymentRequestDTO, String country, String currency) {
        ...
        try {
            ...
            logger.info("The MSISDN is {} ", airtelPaymentRequestDTO.getSubscriber().getMsisdn());
    ...
    public AirtelEnquiryResponseDTO getTransactionStatus(String transactionId, String country, String currency) {
        ...
        try {
            ...
            logger.info("Response: {} status: {}", exchange.getBody(), exchange.getStatusCode());
            return exchange.getBody();
        } catch (HttpClientErrorException.BadRequest ex) {
            try {
                String responseBody = ex.getResponseBodyAsString();
                logger.info("Response body from 404 error: {}", responseBody);
    ...
    public void forwardCallbackRequest(String url, String clientCorrelationId, String message, String statusCode,
        ...
        try {
            restTemplate.exchange(url, HttpMethod.POST, new HttpEntity<>(requestBody, new HttpHeaders()), AirtelPaymentResponseDTO.class);
        } catch (Exception ex) {
            logger.info("Invalid response! {}", ex.getMessage());
        }
    }
}
